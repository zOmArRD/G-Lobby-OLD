...
final class LobbySelector
{
	...
	{
		$this->menu = InvMenu::create(InvMenuTypeIds::TYPE_CHEST)->setName("§l§cGhostly §f» §r§6Lobby Selector")
			->setListener(
	...
	public function addServer(Server $server, int $slot): void
	{
		...
		if ($server->isOnline()) {
			$item->setLore(Ghostly::$server_items->get('Lobby')['online']);
		} else {
			$item->setLore(Ghostly::$server_items->get('Lobby')['offline']);
		}

		if ($server->getName() === Ghostly::SERVER) {
			$item->setLore(Ghostly::$server_items->get('Lobby')['already-connected']);
		}
